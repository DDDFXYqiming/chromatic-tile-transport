...
浮光花园使用独立的 AI 生成人物、鱼、花枝与背景素材。旧五图配置保存在 `examples/matrix-motion/starrail.config.json`。
...
| `rigStrength` | 0 到 1，单层动作比例，乘以全局幅度 |
...
`deformation` 开关局部形变，`deformationStrength` 范围 0 到 1.5。`motionStudy` 固定启用时的镜头、刚性图层姿态和原画模式，只让局部动作随时间改变。减少动态会同时停止局部形变。三角网格与资源管理使用 PixiJS 的几何与资源管理接口，详见 [来源与应用](PIXI_SKILLS.md)。
