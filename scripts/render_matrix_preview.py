...
def main():
    ...
    p.add_argument('--start',type=float,default=0);p.add_argument('--motion-study',action='store_true');p.add_argument('--clean',action='store_true');a=p.parse_args()
    ...
    with tempfile.TemporaryDirectory() as tmp, sync_playwright() as p:
        ...
        try:
            for i in range(round(a.duration*a.fps)):
                page.evaluate('(t)=>MatrixMotion.seek(t)',a.start+i/a.fps);proc.stdin.write(page.screenshot(type='png'))
