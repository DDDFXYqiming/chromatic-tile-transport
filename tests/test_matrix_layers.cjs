...
check('Independent layer motion changes relative positions rather than translating one poster',()=>{
 const a=L.layout(config.composition,scenes[0],.2,1,12,config.options,1440,900);
 const b=L.layout(config.composition,scenes[0],.2,3,12,config.options,1440,900);
 ...
 assert.deepEqual(L.layout(config.composition,scenes[0],.2,1,12,off,1440,900),L.layout(config.composition,scenes[0],.2,3,12,off,1440,900));
});
check('Layer animation is reversible and periodic at the composition loop',()=>{
 const sample=t=>L.layout(config.composition,scenes[0],0,t,12,config.options,1440,900);
 assert.deepEqual(sample(2),sample(2));const a=sample(0),b=sample(12);
