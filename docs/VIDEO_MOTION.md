...
展厅的效果 02 现在进入 [视频版](../dist/matrix-video.html)。两个镜头使用不同构图和动作，均通过实时原画、双色、线描和固定点阵处理。导演台可关闭“画面自身运动”，比较静止首帧与视频，也可进入 [网格对照版](../dist/matrix-motion.html)。视频版默认纯画面，取消纯画面后可看到原有慢速切换卡片。
