...
def main():
    ...
    p.add_argument('--duration',type=float,default=14.2);p.add_argument('--fps',type=int,default=24);p.add_argument('--width',type=int,default=1280);p.add_argument('--height',type=int,default=800);a=p.parse_args()
    ...
    with tempfile.TemporaryDirectory() as tmp, sync_playwright() as p:
        html=build(Path(tmp)/'matrix.html').read_text(encoding='utf-8')
        ...
        try:
            for i in range(round(a.duration*a.fps)):
                page.evaluate('(t)=>MatrixMotion.seek(t)',i/a.fps);proc.stdin.write(page.screenshot(type='png'))
